...
const events = ['cq.local_stats_updated'];
let pushCount = 0;
...
class jitsi_meet_data {
  ...
  updateRes(data, elem) {
    if (
      data !== null &&
      (data.height !== this[elem].height || data.width !== this[elem].width)
    ) {
      this[elem] = data;
      this.updated.push(elem);
    }
  ...
  updateCodec(data, elem) {
    if (
      data !== null &&
      (data.audio !== this[elem].audio || data.video !== this[elem].video)
    ) {
      this[elem] = data;
      this.updated.push(elem);
    }
...
function updateStats(stats) {
  ...
  if (
    ...
    Object.keys(stats.resolution).length > 0
  ) {
    let resolution = null;
    if (stats.resolution.hasOwnProperty(APP.conference.getMyUserId())) {
      resolution =
        stats.resolution[APP.conference.getMyUserId()][
          Object.keys(stats.resolution[APP.conference.getMyUserId()])[0]
        ];
    }
    if (resolution) {
  ...
  if (
    stats.codec &&
    APP.conference.getMyUserId() &&
    Object.keys(stats.codec).length > 0
  ) {
    let codec = null;
    if (stats.codec.hasOwnProperty(APP.conference.getMyUserId())) {
      codec =
        stats.codec[APP.conference.getMyUserId()][
          Object.keys(stats.codec[APP.conference.getMyUserId()])[0]
        ];
    }
    if (codec) {
...
const format_data = (data) => {
  ...
  delete data.conf;
  delete data.uid;
  return { uid, conf, m: { ...data, ...formated_data } };
};
// const format_data = (data) => {
//   let formated_data = {
//     u: {},
...
function pushStats() {
  ...
  if (Object.keys(update).length > 0) {
    ...
    fetch(url, {
      ...
      .then((res) =>
        res.jmmc_objectId
          ? sessionStorage.setItem('jmmc_object_id', res.jmmc_objectId)
          : null,
      )
      .catch((e) => {});
  }
